    ...
    internal sealed class ExpressionParser
    {
            ...
            while (true)
            {
                ...
                switch (tokens.TryGet(index))
                {
                    ...
                    case SyntaxCharacterToken { SyntaxCharacter: SyntaxCharacter.ParenthesisClosing } closingParenthesis:
                        ...
                        return;
                    case Token token:
                        analyser.ReportError(DiagnosticsCode.ExpectedCommaOrClosingParenthesis, token);
                        expression = null;
                        return;
                    case null:
        ...
        private ExpressionNode? ParseSubExpression(ref int index)
        {
            switch (tokens[index])
            {
                ...
                case SyntaxCharacterToken { SyntaxCharacter: SyntaxCharacter.ParenthesisOpening }:
                    {
                        ...
                        Token? nextToken = tokens.TryGet(index);

                        if (nextToken is not SyntaxCharacterToken { SyntaxCharacter: SyntaxCharacter.ParenthesisClosing })
                        {
                            if (nextToken == null)
                            {
                                nextToken = tokens[^1];
                            }
                            analyser.ReportError(DiagnosticsCode.ExpectedClosingParenthesis, nextToken);
                            return null;
                        }
                ...
                case EndOfFileToken endOfFileToken:
                    analyser.ReportError(DiagnosticsCode.ExpectedExpressionTerm, endOfFileToken);
                    return null;
                case var token:
                    analyser.ReportError(DiagnosticsCode.UnexpectedExpressionTerm, token);
                    return null;
            }
        }
